...
function addCons(){
	...
	$sql = "INSERT INTO tbl_picapoint (cons_no, ship_name, phone, s_add, rev_name, r_phone, r_add,  type, weight, invice_no, qty, book_mode, freight, mode, pick_date, pick_time, status, comments, book_date )
	...
	//echo $sql;
	dbQuery($sql);
	header('Location: product_add_success.php'); 
...
function markDelivered() {
	$cid = (int)$_GET['cid'];
	$sql = "UPDATE tbl_picapoint
			SET status = 'Delivered'
			WHERE cid= $cid";
	dbQuery($sql);
	header('Location: delivered_success.php'); 
...
function addNewOffice() {
	...
	dbQuery($sql);
	header('Location: feedmill_add_success.php');
}
...
function addManager() {
	...
	$sql = "INSERT INTO tbl_picapoint_officers (officer_name, off_pwd, address, email, ph_no, office, reg_date)
			VALUES ('$ManagerName', '$Password', '$Address', '$Email', '$PhoneNo', '$OfficeName', NOW())";
	dbQuery($sql);
	header('Location: manager_add_success.php');
...
function updateStatus() {
	...
	$sql = "INSERT INTO tbl_picapoint_track (cid, cons_no, current_city, status, comments, bk_time)
			VALUES ($cid, '$cons_no', '$OfficeName', '$status', '$comments', NOW())";
	dbQuery($sql);
	
	$sql_1 = "UPDATE tbl_picapoint
				SET status = '$status' 
				WHERE cid = $cid
				AND cons_no = '$cons_no'";
	dbQuery($sql_1);	
